...
// ============================================
// Token Functions (จัดการใน token_service.go)
// ============================================
// CreateQRToken, ClaimQRTokenAnonymous, ClaimQRToken,
// ValidateClaimToken, MarkClaimTokenAsUsed อยู่ใน token_service.go แล้ว
...
func SaveCheckInOut(studentId, programId, checkType string) error {
	...
	case "checkin":
		...
		if idx := findTodayCheckinRecord(records, dateKey); idx >= 0 {
	...
	case "checkout":
		...
		idx := findTodayCheckinRecord(records, dateKey)
