...
func GetEnrollmentsByActivityID(
	...
	pipeline = append(pipeline,
		bson.D{{Key: "$group", Value: bson.M{
			...
			// food/registrationDate อาจต่างกันระหว่าง item -> เอา first พอ (ปรับตามธุรกิจได้)
		...
		bson.D{{Key: "$project", Value: bson.M{"checkInOutNested": 0}}},
	)

	// 5) จัดเรียง (รองรับ sortBy เดิม)
	sortDoc := bson.D{}
	order := 1
